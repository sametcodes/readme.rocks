export const getPlatformResponse = async (query: any, services: any, templates: any) => {
    ...
    if (typeof returnType === "string" && ["json", "svg"].includes(returnType) === false) {
        return { success: false, status: 400, error: 'Bad Request: invalid value for returnType parameter' }
    }

    if (Object.keys(services).includes(method) === false) return { success: false, status: 404, error: "Method not found" }

    const service = services[method];
    const template = templates[method];

    if (!service) return { success: false, status: 500, error: "Service not found" };
    if (!template) return { success: false, status: 500, error: "Template not found" };
    ...
    if (returnType === "json") {
        return { success: true, status: 200, data: JSON.stringify(response.data), contentType: "application/json" };
    }
