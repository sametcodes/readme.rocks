...
export const createConfig: DataAPIMethod = async ({ payload, session }) => {
    const platform = await prisma.platform.findFirst({ where: { code: payload.platform } });
    ...
    if (existingConfig) {
        throw new Error("You already have a config for this platform, please update it instead of creating a new one.");
    }
...
export const editConfig: DataAPIMethod = async ({ payload, session }) => {
    ...
    if (!existingConfig) {
        throw new Error("You don't have a config for this platform, please create one first.");
    }
