...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!service) return res.status(400).json({ message: "Bad request: service parameter is missing" });
    if(!method_services.includes(service)) return res.status(400).json({ message: "Bad request: method not allowed for this service" });

    const dataService = services[service];
    if (!dataService) return res.status(400).json({ message: "Bad request: unknown data API service" });
